...
//: - callout(Exercise): After the loop has finished, write an `if` statement that compares the two values and prints a different message based on whether the vote passed or failed.
    if shouldInstallCoffeeVendingMachineVotes{
        yesVotes = yesVotes + 1
    } else {
        noVotes = noVotes + 1
    }
...
func printResults (forlssue:String,withVotes:[Bool]){
var yesVotes = 0
var noVotes = 0
...
print("Result for issue : ", forlssue, "yes: ", yesVotes, "no: ", noVotes)
}




// Add your func printResults(forIssue: String, withVotes: [Bool] ) {
